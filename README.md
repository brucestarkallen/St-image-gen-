...
- **Fixed: the trailing-garment t-shirt.** A world-dress garment welded at the END of a character block lost to the model's priors (a 'black shihakusho' came back a black t-shirt). Welded garments now sit BEFORE the state, emphasis-braced in traditional worlds (`{black shihakusho}`); `appendAnchor` sees through braces, so no tail duplicate.
- Gate: 251 → 264 checks; all three guards negative-tested (exit 1 each).
- **Cast note (user-side, unchanged advice):** `petite` in a cast line + `1girl` count is what renders a small-statured adult as a child. Reword to `short stature` in the sheet.
...
- **Fixed: doubled locative in background demotions** — `seen from behind and below in the background in the background`. Locatives are owned by the wrapper now.
- Gate: 232 → 239 checks; neutralization negative-tested against a reverted scratch copy (exit 1).
- **Style note (needs an A/B, not a guess):** if one strip looks flat/shonen and another looks alive with the same engine, the variables are backend, model, steps/scale, and cast health — the pipeline constants are identical between runs. Per the standing law, no style tag changes without before/after evidence.

### 0.22.0 — the anchor owns the environment, and a demoted figure keeps its verb
- **Fixed: the crowd-eruption frame rendering an EMPTY courtyard.** The builder restated the entire location block (~15 tags) inside every panel's shared prompt, on top of the setting/dress anchor the extension stamps — four frames carried the same courtyard three times each. The crowd-action tags the establishing frame lives or dies by drowned in the restatement, and the triple-stacked atmosphere tags (`dust cloud, motion blur, pale winter sunlight…`) fogged out what little crowd the model attempted. New `dedupeAgainstAnchor()` enforcement, wired into both panel paths: a shared-prompt token that restates an anchor token (exact, or every word already anchor-owned) is dropped, and self-duplicates collapse. Environment facts have exactly one owner — the anchor — stamped once per panel by `appendAnchor`. Camera, lighting, and action phrases that merely share words with the anchor survive.
- **Fixed: a principal stealing the background figure's action.** `splitPrincipals()` demoted a distant figure to the constant string `distant figure in the background`, discarding its state — the raised sky-blue blade floated free in the shared prompt and the model bound it to the only person left in the frame (field: the saluting old soldier rendered HOLDING the sword, in black, because the blade's colour died with the discarded state). New `backgroundFigureTag()` carries the demoted figure's action along (`distant figure holding sword overhead, sky-blue blade in the background`), scrubbed of appearance and garments by the figure's own cast block — a background figure gets no second identity, only its verb.
- Gate: 217 → 232 checks; the new guards negative-tested against a reverted scratch copy (exit 1), and the wiring is pinned by source-contract checks.
- **Not changed (deliberately):** the `1girl`/`1boy` count forms for adult cast entries — danbooru convention is load-bearing — and no new tags were added by reasoning. If an adult character renders young, adjust the cast sheet (e.g. replace `petite` with height/build wording) rather than stacking stature words.

### 0.21.0 — the `crowd` tag was the reason crowds were grey (reverts 0.14.0/0.15.0)
- **Reverted: the `crowd` count tag.** 0.14.0 added it on the reasoning that `1boy, 1girl` told the model only two people existed and everyone else degraded into mass. A field A/B across the same scene says the opposite: builds *without* the tag returned crowds with faces, open mouths, and raised fists; every build *with* it returned grey featureless blobs in every panel. `crowd` is the danbooru tag for indistinct background mass — it was asking for exactly what came back. The population is carried by the setting's own words ("packed courtyard of shinigami in black shihakusho, officers on roof tiles"), which the laws already mandate and which never enters the count run.
- **Reverted: the bound `crowd in <garment>` phrase** from 0.15.0, for the same reason — it restated the setting's own population dress and flattened three hundred people into one uniform mass. The bug it was written for (anchor dedup deleting the world dress) is covered by the setting, which dedup never touches.
- **Fixed: an establishing frame rendering an empty courtyard.** A crowd-subject panel was given `crowd` and nothing else. That is a mood tag, not a headcount, so nobody appeared. It now carries `6+boys, 6+girls, crowd` — the danbooru tags that actually populate a frame.
- **Fixed: an adult rendering with child proportions.** Stature was being stated three times — `petite` in the cast block, `small posture` in the state, "small and straight" in the sentence — and the model stacked them. The cast sheet owns stature; a state that merely repeats it is dropped, while a real action that happens to contain a size word ("small smile playing at her mouth") survives.
- Gate: 215 → 217 checks; all seven new guards negative-tested.
...
- **Reverted: character name tags. They were a mistake.** 0.17.0 had auto-build lead a canon character's line with their danbooru tag and keep the body description after it — two competing identity specs in one block. The image model draws its own idea of the character, that fights the body you described, and the two blend into something that is neither; the field run returned a Renji with a woman's chest. Identity comes from the body and nothing else: sex, build, hair, eyes, marks, clothing. Name tags and their work tags are also stripped from existing cast sheets at the weld, so no rebuild is needed to undo it.
- Gate: 201 → 208 checks; all nine new guards negative-tested. (0.19.1 removes a function left dead by the merge and the check that covered it.)

### 0.18.0 — the strip is planned and checked before a single tag is written
- **New: two-pass building.** One call used to choose the beats, order them, decide who was in each frame, derive the world, and write every tag, under seven thousand characters of law. Every field failure of the last six versions was a *planning* failure — a beat spent twice, two people packed into one frame, an all-solo strip, a population nobody named — and no amount of code could catch them, because the plan arrived already fused to its own rendering. The strip is now laid out first in plain language (`panel 1 — beat / who`), small enough to read and check, and only then rendered into tags. The render pass is told the beats and the cast of every frame are settled and its only job is the tags.
- **The plan is validated in code and repaired once if it fails.** Caught before any image exists: the same beat spent on two panels (content-word overlap), a name that is not in the cast sheet, a panel missing its `who` field, an all-solo strip when the scene has people acting on each other, and a crowd scene with no frame given to the crowd. Each problem is handed back as a specific instruction for one repair call. If the plan pass fails or its output will not parse, the builder falls back to the old single call — the image is never blocked on it.
- **Fixed: a lieutenant rendering as WWII military dress.** Her cast line said `lieutenant's badge`, and an image model reads a rank word beside a decoration word as collar tabs, shoulder boards, and an eagle. `filterRankGarments` had cleaned the world dress for versions but never the cast block. Rank *decorations* are now stripped at the weld itself — no cast rebuild needed — while a garment that merely belongs to a rank (a captain's haori) survives untouched. The negative also carries the specific composite the model kept reaching for.
- **The plan is the first line of Show last generation**, with any repairs it needed. When a strip comes out wrong, the plan says whether the layout was wrong or the rendering was.
- Gate: 185 → 201 checks; all ten new guards negative-tested, including one that initially passed against a broken build and was rewritten to test the weld rather than the helper.

### 0.17.0 — one rule per decision, and canon characters get their real name
- **Fixed: every panel a solo portrait.** Six separate rules were negotiating who goes in a frame, and two of them pulled opposite ways — 0.16.0's "padding a frame with a second principal is a failed panel" against 0.12.5's "defaulting everything to solo is a failed strip." The builder resolved the contradiction the safe way and made every panel one person, so a strip whose whole subject is people reacting to each other contained no relationship at all. All six are now ONE rule: `who` is the people the beat's action passes between — both when the beat is between two, one when it happens inside one, `[]` when it belongs to the crowd, never padded to two and never cut to one. The counter-rule is deleted rather than counter-balanced.
- **Canon characters now carry their danbooru character tag.** An anime image model already knows what a published character looks like and what they wear; their own tag invokes that knowledge and binds harder than any description while never drifting between panels. Auto-build now leads a canon character's line with `kuchiki rukia, bleach` and keeps the description after it. Original characters get description only — the extension never invents a character tag. **Re-run Auto-build cast to pick this up on existing casts.**
- **Fixed: rank words in a cast line rendering as modern military dress.** `filterRankGarments` cleaned the world dress but never the cast sheet, so a character tagged with a rank insignia came back in gold cuff braid and shoulder boards. Auto-build now writes the garment that is visible ("white armband on left arm"), never the rank it signifies.
- **Fixed: an accent quietly costing us the garment.** Danbooru tags are ASCII: `shihakusho` is a trained token and the same word with a macron is not, so the model fell back to a generic black shirt. Latin diacritics are folded at prompt composition; CJK and other scripts are untouched.
- **Fixed: panels ending mid-word.** The composition sentence was hard-sliced at 220 characters and the field run ended a panel on "as the ," — a dangling fragment the model has to interpret. It now cuts at a sentence end, else a word boundary, with trailing punctuation removed.
- **Strips stop fighting their own continuity at the API.** The NovelAI request sent `variety_boost: true`, which exists to push outputs apart — the runtime twin of the per-panel seed problem fixed in 0.15.0. It is off for strips and kept for single frames. The dress anchor also landed up to three times per panel; it lands once now.
- Gate: 176 → 185 checks; all nine new guards negative-tested.

### 0.16.0 — the crowd can be the subject, and two prose-only laws got their enforcers
- **Fixed: a lieutenant rendered in a modern military uniform.** 0.15.0 added "never a garment in `state`" to the laws and shipped no code to enforce it — the exact mistake the AGENTS note warns against. The builder wrote `shinigami uniform` into a character's state, it landed beside the shihakushō the code had welded, and the model blended two outfits into WWII army dress. Garment tags are now stripped from `state`, with one deliberate exception: a garment token carrying a condition word (torn, open, removed, blowing) describes what happened *to* the clothing rather than naming an outfit, and the explicit-scene rules depend on it.
- **Fixed: "dispersing crowd" stamped onto a scene whose courtyard is erupting.** 0.14.0 forbade transient verbs in `setting` in prose and, again, shipped no enforcer. `setting` is stamped unchanged onto every panel, so one participle contradicts the whole strip. The activity is now stripped and the population and its dress kept — `dispersing crowd of shinigami in black shihakushō` becomes `crowd of shinigami in black shihakushō`.
- **A frame may now have no named principal.** `parsePanels` flattened an omitted `who` and an explicitly empty `who` to the same value, so the corrective retry could not tell "the builder ignored the schema" from "this frame's subject is the crowd" — and punished both. A scene whose emotional core is three hundred people roaring could not be given a single frame of three hundred people roaring; every panel had to be a face. An explicit `"who": []` is now a legal establishing frame that still receives the population count and shot grammar, while an omitted `who` remains non-compliance and still draws the one corrective retry.
- **One beat per panel.** Two beats crammed into a frame ("X salutes by the stone while Y shouts across the courtyard") fails twice: a single prompt cannot bind two actions to two separate bodies, and the model fuses them. Beats that do not fit the panel budget are dropped, not merged — and a sword leaving its sheath followed by that same sword held overhead is one beat rendered twice, which wastes a panel the scene needed elsewhere.
- Gate: 167 → 176 checks; all nine new guards negative-tested.

### 0.15.0 — counts and clothing are code-owned, and the strip stays in one place
- **Fixed: two characters fusing into one.** The builder was writing count tags into each character's `state`, so the welded prompt read `2boys, <first block>, 1boy, ..., <second block>, 1boy`. A count tag inside a block is a second subject declaration mid-prompt: it breaks the contiguous run the whole weld depends on, and the model cross-binds the blocks around it — the field run returned one old man with white hair holding the sword, with the second character gone. Counts are computed by the extension and are now stripped from any field the builder can write to.
- **Fixed: crowds and principals in modern clothes.** The world's dress was stamped as bare garment tags, so `appendAnchor`'s dedup deleted it from exactly the panels where a principal already wore that garment — taking the *crowd's* only clothing instruction with it. One panel of the field run lost it entirely and three hundred shinigami came back in school uniforms. The dress anchor is now bound to the population (`crowd in <garment>`), a phrase no character block can collide with. Separately, a principal whose cast entry contains no garment at all is now dressed by code from the world dress instead of by the model's priors, and the anti-modern negative covers the school-uniform prior and modern architecture — the two defaults a tag model reaches for when a scene is Japanese and nobody said otherwise.
- **Fixed: every panel a different building.** Panel seeds were derived from who was in the frame (0.12.5) to stop palette priors bleeding between different subjects — from when the *builder* wrote appearance. Identity has been welded verbatim from the cast sheet since 0.13.0, which pins subject appearance far harder than seed decorrelation ever did, while nothing pinned the location: a fresh seed per panel re-rolled the architecture of a strip that is one continuous place. Welded panels now share the run seed; the who-hash remains the backstop for panels whose identity the code does not own, so 0.12.5's protection is intact exactly where it still applies.
- **Fixed: two framing tags in one panel.** The laws mandate exactly one framing tag and one angle tag; nothing enforced it, and `low angle, full body, wide shot` reached the model, which split the difference into unreadable figures. `enforceShotGrammar` keeps the first of each kind.
- **Solo beats stay solo.** The two-shot rule was reading as "fill both slots", so panels were padded with a second principal doing something unrelated elsewhere in the location — a frame a single prompt cannot bind. A beat that belongs to one person is now explicitly a solo frame.
- Gate: 157 → 167 checks; all nine new guards negative-tested.
