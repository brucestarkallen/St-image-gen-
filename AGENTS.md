...
1a0000. IDENTITY IS CODE: named-character appearance is inserted by assembleIdentity from the cast sheet via each panel's `who` list — the builder is contractually and structurally barred from writing named characters' looks. Never regress identity to builder-written prompt text; every identity bug class this session (substitution, omission, fusion, drift) came from that. Unknown who-names are warned and skipped, never guessed (negative-tested).
